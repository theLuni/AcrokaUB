...
def get_module_info(module_name):
    ...
    try:
        ...
        with open(module_path, 'r', encoding='utf-8') as f:
            ...
            # Инициализируем переменные для информации о модуле
            name = about = commands = by = 'Неизвестно'

            # Анализируем каждую строку на наличие информации
            for line in lines:
                if line.startswith("#name:"):
                    name = line[len("#name:"):].strip()
                elif line.startswith("#about:"):
                    about = line[len("#about:"):].strip()
                elif line.startswith("#commands:"):
                    commands = line[len("#commands:"):].strip()
                elif line.startswith("#by:"):
                    by = line[len("#by:"):].strip()
...
def get_loaded_modules():
    # Функция для получения списка загруженных модулей
    # Здесь вы можете имплементировать логику как хотите
    return [f[:-3] for f in os.listdir(MODS_DIRECTORY) if f.endswith('.py')]
