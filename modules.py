...
class ModuleManager:
    ...
    async def load_module(self, module_name: str) -> bool:
        try:
            ...
            if await self._check_dependencies(module_path):
            ...
            traceback.print_exc()
            # Удаляем файл модуля при ошибке загрузки
    ...
    async def _check_dependencies(self, module_path: str) -> bool:
        ...
        try:
            ...
            # Ищем строки с зависимостями (например: # requires: requests, pytz)
            ...
            print(f"❌ [Ошибка] Проверка зависимостей: {str(e)}")
            return False
...
class CoreCommands:
    ...
    def get_cpu_usage(self):
        ...
        try:
            return f"{psutil.cpu_percent()}% ({psutil.cpu_count()} ядер)"
        except:
            return "N/A"
    ...
    async def handle_info(self, event: Message):
        ...
        for mod_name, mod_data in self.manager.modules.items():
            ...
            mod_stats.append(
                f"• <b>{mod_name}</b> - загружен {uptime.seconds // 3600}ч {(uptime.seconds % 3600) // 60}м"
            )
        ...
        info_msg = [
            ...
            *mod_stats if mod_stats else ["ℹ️ Нет загруженных модулей"],
            "",
    ...
    async def handle_loadmod(self, event: Message):
        ...
        try:
            ...
            if await self.manager.load_module(module_name):
                ...
                info_msg = [
                    f"✅ <b>Модуль {module_name} успешно загружен!</b>",
                    f"📝 <b>Описание:</b> {desc}",
